...
1. **API Transferência** realiza transferência entre contas
2. Publica mensagem no **Tópico Kafka: Transferências Realizadas**
3. **Aplicação Tarifas** consome a mensagem
4. Persiste tarifa no **Banco Tarifas**
5. Publica mensagem no **Tópico Kafka: Tarifas Realizadas**
6. **API Conta Corrente** consome e debita o valor da tarifa
...
|-------|------|-----------|
| **idcontacorrente** | INT | PK - Identificador único |
| numero | VARCHAR | Número da conta |
| nome | VARCHAR | Nome do titular |
| ativo | BOOLEAN | Status da conta (1=ativa, 0=inativa) |
| senha | VARCHAR | Senha (hash) |
| salt | VARCHAR | Salt para criptografia |

#### Tabela: movimento
| Campo | Tipo | Descrição |
|-------|------|-----------|
| **idmovimento** | INT | PK - Identificador único |
| idcontacorrente | INT | FK - Referência à conta |
| datamovimento | DATETIME | Data e hora do movimento |
...
| Campo | Tipo | Descrição |
|-------|------|-----------|
| **chave_idempotencia** | VARCHAR | PK - Chave única da requisição |
| requisicao | TEXT | Dados da requisição |
| resultado | TEXT | Resultado armazenado |

**Relacionamento:** `contacorrente.idcontacorrente` ← `movimento.idcontacorrente` (1:N)
...
| Campo | Tipo | Descrição |
|-------|------|-----------|
| **idtransferencia** | INT | PK - Identificador único |
| idcontacorrente_origem | INT | FK - Conta origem |
| idcontacorrente_destino | INT | FK - Conta destino |
| datamovimento | DATETIME | Data e hora da transferência |
...
### Schema: tarifas (OPCIONAL)

#### Tabela: tarifas
| Campo | Tipo | Descrição |
|-------|------|-----------|
| **idtarifa** | INT | PK - Identificador único |
| idcontacorrente | INT | FK - Conta tarifada |
| datamovimento | DATETIME | Data e hora da tarifação |
...
### Efetuar transferência entre contas da mesma instituição
...
### Criar uma Aplicação para Tarifa
...
Neste caso deve ser adicionada mais uma etapa (após o sucesso das duas chamadas para conta corrente) na API Transferência para produzir uma mensagem neste tópico de transferências realizadas, enviando a identificação da requisição e a identificação da conta corrente logada.

Além disso, também será preciso implementar o consumo do tópico Kafka de tarifações realizadas na API Conta Corrente implementando o mesmo funcionamento do serviço movimentação na conta corrente (Sempre debitando o valor tarifado).
